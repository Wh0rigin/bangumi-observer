    ...
    try {
        ...
        posts.each((_, post) => {
            ...
            futureIntroList.each((index, intro) => {
                ...
                while (
                    nextElement &&
                    nextElement.prop("tagName") !== "P" &&
                    !nextElement.hasClass("future_intro")
                ) {
                    if (
                        nextElement.prop("tagName") === "DIV" &&
                        nextElement.find("td.future_title_").length > 0
                    ) {
                        const futureTitle = nextElement
                            .find("td.future_title_")
                            .text()
                            .trim();
                        const futureDate = nextElement
                            .find('p[class^="future_date"]')
                            .text()
                            .trim();
                        const futureType = nextElement
                            .find('p[class^="future_type_"]')
                            .text()
                            .trim();
